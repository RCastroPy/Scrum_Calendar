(() => {
  ...
  async function loadBase() {
    ...
    const [sprints, cells, personas, eventos, tipos, feriados, sprintItems, timeInfo, usuarios] = await Promise.all([
      fetchJson("/sprints"),
      fetchJson("/celulas"),
      fetchJson("/personas"),
      fetchJson("/eventos"),
  ...
  function initForms(base) {
    ...
    celulaForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        const payload = { nombre: celulaForm.nombre.value.trim(), activa: true };
  ...
  function renderAdminTable(container, rows, columns, actions = [], tailColumns = []) {
    ...
    const pageSize = Number(container.dataset.pageSize || defaultPageSize) || 0;
  ...
  function applyHeatmapClasses(rows, keys) {
    const ranges = {};
    keys.forEach((key) => {
      const values = rows
  ...
  function buildSprintItemStats(items) {
    ...
    items.forEach((item) => {
      ...
      if (item.persona_id) {
        const stats = ensureStats(byPersona, item.persona_id, "");
        stats.totalTareas += 1;
  ...
  async function renderDaily(base) {
    ...
    if (sprintSelect) {
      fillSelect(sprintSelect, sprints);
      if (selectedSprint) {
    ...
    const buildTotalDaysCell = (row) => {
      ...
      const expected = Number.isFinite(points) ? refDaysByPoints.get(points) : null;
    ...
    const itemsAll = base.sprintItems.filter((item) => {
      ...
      if (item.persona_id && !activePersonaIds.has(item.persona_id)) {
        return false;
      }
    ...
    if (selectedPersonaId) {
      items = items.filter((item) => String(item.persona_id) === String(selectedPersonaId));
    ...
    const currentStats = buildSprintItemStats(itemsAll);
    const previousStats = buildSprintItemStats(previousItems);
    const previousStats2 = buildSprintItemStats(previousItems2);
    ...
    applyHeatmapClasses(teamRows, heatmapKeys);
    ...
    renderAdminTable(
      ...
      [
        ...
        },
        {
          ...
          getValue: (row) => personaMap[row.persona_id] || row.assignee_nombre || "",
        },
      ],
      [
        {
    ...
    if (form) {
      const personaOptions = personasFiltradas.map((persona) => ({
        id: persona.id,
        nombre: `${persona.nombre} ${persona.apellido}`,
      }));
      fillSelect(form.persona_id, personaOptions);
      fillSelect(form.sprint_id, sprints);
      if (selectedSprint) {
  ...
  function initDaily() {
    ...
    if (!state.dailyStatusOutsideBound) {
      ...
      document.addEventListener("click", () => {
        if (!state.dailyStatusOpen) return;
        state.dailyStatusOpen = false;
        renderDaily(state.base);
      });
    }
  ...
  function renderAdmin(base) {
    ...
    const celulasBuscadas = (state.selectedCelulaId
      ...
      : base.cells
    ).filter((celula) => matchesQuery([celula.id, celula.nombre]));
          ...
          ? [
              {
                ...
                onClick: (row) => {
                  ...
                  celulaForm.scrollIntoView({ behavior: "smooth", block: "center" });
                },
              },
            ]
          : []),
      ...
      [
        ...
        ...(canEditPersona
          ? [
            ...
            if (sinCelulaToggle) {
              sinCelulaToggle.checked = !selected.size;
              syncPersonaCelulas(!selected.size);
            }
            if (personaForm.activo) {
              personaForm.activo.value = row.activo ? "true" : "false";
            }
                  setFormMode(personaForm, "edit", row.id, "Actualizar persona");
                  personaForm.scrollIntoView({ behavior: "smooth", block: "center" });
                },
              },
            ]
          : []),
          ...
          ? [
              {
                ...
                onClick: (row) => {
                  ...
                  sprintForm.scrollIntoView({ behavior: "smooth", block: "center" });
                },
              },
            ]
          : []),
          ...
          ? [
              {
                ...
                onClick: (row) => {
                  ...
                  feriadoForm.scrollIntoView({ behavior: "smooth", block: "center" });
                },
              },
            ]
          : []),
          ...
          ? [
              {
                ...
                onClick: (row) => {
                  ...
                  tipoForm.scrollIntoView({ behavior: "smooth", block: "center" });
                },
              },
            ]
          : []),
          ...
          ? [
              {
                ...
                onClick: (row) => {
                  ...
                  eventoForm.scrollIntoView({ behavior: "smooth", block: "center" });
                },
              },
            ]
          : []),
  ...
  async function initPokerPublic(options = {}) {
    ...
    ensurePokerSocket(token, "public", () => {
      loadInfo();
    });
